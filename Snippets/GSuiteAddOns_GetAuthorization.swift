...
func sample(client: GSuiteAddOnsClient, projectId: String) async throws {
  let response = try await client.getAuthorization(
    request: GetAuthorizationRequest()
      .with {
        $0.name = "projects/\(projectId)/authorization"
      }
  )
